...
| HBOX_OPTIONS_PRODUCT_LOOKUP             | true                                                                       | look an unknown scanned barcode up at OpenFoodFacts to suggest a product name. Only the barcode digits are sent, and only when a user scans a code no local item carries. Set to false to keep every scan on your own server. |
